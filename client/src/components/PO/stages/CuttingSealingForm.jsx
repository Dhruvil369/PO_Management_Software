...
const CuttingSealingForm = ({ onComplete, onBack, machineData, initialData }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    console.log('CuttingSealingForm - Form data:', formData);
    ...
    try {
      ...
      if (onComplete) {
        ...
        console.log('CuttingSealingForm - Data submitted successfully');
      } else {
        throw new Error('onComplete function not provided');
      }
    } catch (error) {
      console.error('CuttingSealingForm - Submit error:', error);
      setError('Failed to save cutting & sealing data: ' + (error.message || 'Unknown error'));
    } finally {
      setLoading(false);
    }
  };
